...
for gamma=0%-50%:10:90
  for beta=0%-90%:10:90
    for alpha=0%-150%:10:150
    ...
    for i=1:limit(1)
       T=RobotFK_Test(radtodeg(unique_solutions(i,:)));
       [x y z]=MyTransl(T);
       v(i,:)=[x y z];      
    end
   ...
   finalsolutions=flipud(finalsolutions);
   
   %% Display the data
   disp(['The desired solution is: ', num2str(alpha),' ', num2str(beta),' ', num2str(gamma)])
   disp(['The position of the end effector is: ', num2str(xe),' ', num2str(ye),' ', num2str(ze)])
   
   limit=size(finalsolutions);
   
   for i=1:limit(1)
        disp(['Final Soltution is: ', num2str((finalsolutions(i,:)))]) 
   end
   disp('----------------------------------------------------------------------------------------------------------------------------------------------------------')
